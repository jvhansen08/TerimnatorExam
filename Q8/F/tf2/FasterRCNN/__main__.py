...
def predict_all_directory(model, input_path, output_path):
  from .datasets.image import load_image
  if not os.path.exists(output_path):
    os.makedirs(output_path)
  print("Rendering predictions from '%s' to '%s'..." % (input_path, output_path))
  for filename in os.listdir(input_path):
    if not filename.lower().endswith(".jpg") and not filename.lower().endswith(".png"):
      continue
    input_file = os.path.join(input_path, filename)
    output_file = os.path.join(output_path, filename)
    image_data, image, _, _ = load_image(url = input_file, min_dimension_pixels = 600)
    _predict(model = model, image_data = image_data, image = image, show_image = False, output_path = output_file)
...
if __name__ == "__main__":
  ...
  group.add_argument("--predict-all-directory", metavar = "dir", action = "store", type = str, help = "Run inference on all images in the specified directory and write to directory 'predictions_${dir}'")
  ...
  elif options.predict_all_directory:
    predict_all_directory(model= model, input_path = options.predict_all_directory, output_path = "./detections")
